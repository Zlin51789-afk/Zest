...
为降低首字延迟：**智能问答**在命中 `faq.json` 关键词时由服务端直接返回答案（需同一词条下至少命中两个关键词，或命中不少于 3 字的单个关键词，避免「技术」等单字误触）；**进度查询**在已匹配到项目 JSON 时由服务端格式化为摘要，上述路径均**不调用**大模型。其余问题仍走 Kimi / OpenClaw。
