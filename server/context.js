...
let qaCtxCache = { mtimeMs: -1, value: null };
...
export async function buildDocumentContext() {
  const now = Date.now();
  if (docsCtxCache.value && now < docsCtxCache.expires) {
    return docsCtxCache.value;
  }
  ...
  docsCtxCache = { expires: now + DOCS_CTX_TTL_MS, value };
  return value;
}
...
export async function buildQaContext() {
  ...
  if (qaCtxCache.value && qaCtxCache.mtimeMs === mtimeMs) {
    return qaCtxCache.value;
  }
  ...
  const value = { faq, knowledgeText: lines.join('\n') };
  qaCtxCache = { mtimeMs, value };
  return value;
}
...
4. 不要用「已找到 N 个文档」「共检索到」等套话开头，直接回答；简洁专业中文，优先 120 字以内。`,
...
规则：回答精准、简洁，使用中文；优先 200 字以内先给结论。不要说明「已匹配几条知识」「找到多少文档」等检索过程，直接给出结论与要点。`,

  progress: (ctx) =>
    `你是「进度查询」智能体，通过 OpenClaw 为用户服务。
请根据以下项目状态数据回答用户的进度查询。使用清晰结构：项目名称、当前阶段、整体进度百分比、里程碑列表（含状态图标 ✓ 完成 / ◐ 进行中 / ○ 待开始）、备注。优先 220 字以内；用户明确要求细节时再展开。
...
使用简洁、友好的中文；默认 180 字以内先答要点，需要展开时再补充。`,
};
